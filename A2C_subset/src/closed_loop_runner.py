...
from A2C_subset.policies.a2c_policy import A2CPolicy
from A2C_subset.policies.a2c_observation_adapter import convert_tread_obs_to_a2c_obs
...
class A2CEgoVehicle(MDPVehicle):
    """highway-env MDP vehicle controlled by a loaded A2C policy."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.last_a2c_action_index = 1
        self.last_a2c_raw_action_index = 1
        self.last_a2c_action_name = "IDLE"

    def _observation(self) -> np.ndarray:
        return convert_tread_obs_to_a2c_obs(
            self,
            None,
            {
    ...
    def _action_name(self) -> str:
        raw_action_index = int(self.policy.act(self._observation()))
        self.last_a2c_raw_action_index = raw_action_index
        self.last_a2c_action_index = raw_action_index
        self.last_a2c_action_name = A2C_ACTIONS.get(raw_action_index, "IDLE")
        return self.last_a2c_action_name
...
class ClosedLoopFollowingRunner:
    ...
    def __init__(
        ...
        self.a2c_policy = self._make_a2c_policy()

    def _make_a2c_policy(self) -> A2CPolicy:
        policy_cfg = dict(self.config.get("a2c_policy", {}) or {})
        ...
        return A2CPolicy(
            ...
            device=str(
                policy_cfg.get(
                    "device",
                    self.config.get("training", {}).get("device", "auto"),
                )
            ),
        )
            ...
            trace.append(
                {
                    ...
                    "ego_a2c_action_index": float(
                        getattr(ego, "last_a2c_action_index", 1)
                    ),
                    "ego_a2c_raw_action_index": float(
                        getattr(ego, "last_a2c_raw_action_index", 1)
                    ),
