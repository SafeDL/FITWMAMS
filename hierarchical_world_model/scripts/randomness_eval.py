...
def main() -> None:
    ...
    parser.add_argument("--samples", type=int, default=RANDOMNESS_ABLATION["samples"])
    ...
    checkpoint = Path(config["paths"]["evaluation_checkpoint"])
    ...
    def evaluate(model, plans: list[np.ndarray], response_random: bool) -> dict:
        samples = [
            rollout(
                ...
                history_frames=25,
                motion_seed=(
                    motion_seed + RANDOMNESS_ABLATION["motion_seed_offset"]
                    if response_random
                    else None
                ),
            )
    ...
    fixed_plans = [fixed_plan] * int(args.samples)
    quadrants = {
    ...
    gates = {
        "energy_improvement_at_least_5pct": energy_improvement >= RANDOMNESS_ABLATION_GATES["energy_improvement_min_fraction"],
        ...
        ] >= RANDOMNESS_ABLATION_GATES["trajectory_pairwise_min_m"],
        "terminal_pairwise_at_least_5cm": full[
            "terminal_pairwise_distance_m"
        ] >= RANDOMNESS_ABLATION_GATES["terminal_pairwise_min_m"],
        "speed_ax_degradation_within_10pct": all(
            value <= RANDOMNESS_ABLATION_GATES["speed_ax_degradation_max_ratio"] for value in distribution_degradation.values()
        ),
        "windowed_jerk_degradation_within_10pct": all(
            value <= RANDOMNESS_ABLATION_GATES["windowed_jerk_degradation_max_ratio"] for value in windowed_jerk_degradation.values()
        ),
