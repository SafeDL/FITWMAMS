...
from PPO_subset.policies.ppo_policy import PPOPolicy
from PPO_subset.policies.ppo_observation_adapter import convert_tread_obs_to_ppo_obs
...
class PPOEgoVehicle(MDPVehicle):
    """highway-env MDP vehicle controlled by a loaded PPO policy."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.last_ppo_action_index = 1
        self.last_ppo_raw_action_index = 1
        self.last_ppo_action_name = "IDLE"

    def _observation(self) -> np.ndarray:
        return convert_tread_obs_to_ppo_obs(
            self,
            None,
            {
    ...
    def _action_name(self) -> str:
        ...
        self.last_ppo_raw_action_index = raw_action_index
        self.last_ppo_action_index = raw_action_index
        self.last_ppo_action_name = PPO_ACTIONS.get(raw_action_index, "IDLE")
        return self.last_ppo_action_name
...
class ClosedLoopFollowingRunner:
    ...
    def __init__(
        ...
        self.ppo_policy = self._make_ppo_policy()

    def _make_ppo_policy(self) -> PPOPolicy:
        policy_cfg = dict(self.config.get("ppo_policy", {}) or {})
            ...
            trace.append(
                {
                    ...
                    "ego_ppo_action_index": float(
                        getattr(ego, "last_ppo_action_index", 1)
                    ),
                    "ego_ppo_raw_action_index": float(
                        getattr(ego, "last_ppo_raw_action_index", 1)
                    ),
