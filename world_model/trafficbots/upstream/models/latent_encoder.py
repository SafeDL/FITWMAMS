        ...
        if not self.dummy:
            if ag_encoder["temp_window_size"] > 0:
                if self.temporal_indices is not None:
                    temp_window_size = len(self.temporal_indices)
                elif self.temporal_down_sample_rate > 1:
        ...
        if self.dummy:
            ...
            if self.temporal_indices is not None:
                indices = torch.as_tensor(
                    self.temporal_indices, device=ag_valid.device, dtype=torch.long
                )
                if (
                    indices.numel() == 0
                    or int(indices[0]) != 0
                    or int(indices[-1]) != ag_valid.shape[-1] - 1
                ):
                    raise ValueError(
                        "posterior temporal indices must include the first and final state"
                    )
                ag_valid = ag_valid.index_select(2, indices)
                ag_motion = ag_motion.index_select(2, indices)
                ag_pose = ag_pose.index_select(2, indices)
                tl_state = tl_state.index_select(2, indices)
            elif self.temporal_down_sample_rate > 1:
