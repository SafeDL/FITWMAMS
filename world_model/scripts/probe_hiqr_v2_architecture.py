#!/usr/bin/env python3
"""Run B0, hierarchical-latent and continuation probes for one HiQR-v2 model."""
...
from world_model.src.core.initial_behavior_anchor import summarize_first_second_states  # noqa: E402
...
def _original_b0_consistency(model, batch: dict, rollout: dict) -> torch.Tensor:
    ...
    return (error * summary_valid.float()).sum() / summary_valid.float().sum().clamp_min(1.0)
...
def _b0_probe(model, loader, device: torch.device) -> dict:
    ...
    for values in loader:
        ...
        for name in rows:
            current = dict(batch)
            if name == "zero":
                current["behavior_anchor_raw"] = torch.zeros_like(batch["behavior_anchor_raw"])
            elif name == "shuffle":
            ...
            rollout = model.rollout_reconstruction(current, response_steps=5, deterministic=True)
    ...
    return {
        name: {"fde_1s_m": float(np.mean(item["fde"])), "original_b0_summary_loss": float(np.mean(item["summary"]))}
        for name, item in rows.items()
    }


def _pair_distance(left: torch.Tensor, right: torch.Tensor) -> float:
    return float(torch.linalg.vector_norm(left[..., :2] - right[..., :2], dim=-1).mean().cpu())
...
def _latent_probe(model, batch: dict, samples: int = 8) -> dict:
    ...
    state = model.initialize_start(
        current, valid, ego, batch["map_polylines"], batch["map_polyline_valid"],
        batch["behavior_anchor_raw"], batch["behavior_anchor_valid"],
    )
    shape_g = (len(current), model.cfg.scene_latent_dim)
    shape_z = (len(current), current.shape[1], model.cfg.agent_residual_dim)

    def trajectory(scene_noise: torch.Tensor, residual_noise: torch.Tensor) -> torch.Tensor:
        return model.plan_step(
            None, None, current, valid, ego, batch["map_polylines"], batch["map_polyline_valid"],
            filter_state=state, deterministic=False,
            scene_standard_normal=scene_noise, agent_standard_normal=residual_noise,
    ...
    for seed in range(samples):
        ...
        first_z, second_z = torch.randn(shape_z, generator=generator, device=current.device), torch.randn(shape_z, generator=generator, device=current.device)
        fixed_scene.append(_pair_distance(trajectory(zeros_g, first_z), trajectory(zeros_g, second_z)))
        first_g, second_g = torch.randn(shape_g, generator=generator, device=current.device), torch.randn(shape_g, generator=generator, device=current.device)
        changed_scene.append(_pair_distance(trajectory(first_g, first_z), trajectory(second_g, second_z)))
    return {
...
@torch.no_grad()
def _continuation_probe(model, batch: dict) -> list[dict]:
    current, valid = (
        batch["agent_states"][:, model.cfg.anchor_state_index],
        batch["agent_valid"][:, model.cfg.anchor_state_index],
    )
    ...
    first = model.plan_step(
        None, None, current, valid, ego, batch["map_polylines"], batch["map_polyline_valid"],
        filter_state=state, deterministic=True,
    )
    realized, history = current.clone(), [current]
    for frame in range(model.cfg.execute_frames):
        physical = current.new_zeros((len(current), current.shape[1], 2))
        physical[:, 1:] = first["background_future_actions"][:, frame]
        realized = model.dynamics.step(realized, physical, valid, model.cfg.simulation_dt_s)
        history.append(realized)
    history_tensor = torch.stack(history, dim=1)
    history_valid = valid[:, None].expand(-1, len(history), -1)
    rows = []
    for offset in (0.0, 0.5, 1.0, 2.0, 4.0):
        perturbed = realized.clone()
        perturbed[:, 0, 0] += offset
        output = model.plan_step(
            history_tensor, history_valid, perturbed, valid, ego,
            batch["map_polylines"], batch["map_polyline_valid"],
            filter_state=first["filter_state"], previous_buffer=first["background_future_actions"],
            previous_current=current, previous_background_states=first["background_future_states"],
            previous_expected_ego=first["expected_ego_states"], slow_scene=first["slow_scene"],
            response_index=1, deterministic=True,
        )
        masks, plan = output["background_future_action_masks"], output["background_future_actions"]
        rows.append({
            "ego_offset_m": offset,
            "emergency_rate_first_5": float(masks["emergency"][:, :5].float().mean().cpu()),
            "middle_gate_mean": float(output["continuation_gate"][:, 5:20].mean().cpu()),
            "first_5_plan_change": float((plan[:, :5] - first["background_future_actions"][:, 5:10]).abs().mean().cpu()),
        })
    return rows
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "world_model/scripts/configs/highd_hiqr_v2_world_model.yaml")
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--max-sequences", type=int, default=256)
    args = parser.parse_args()
    config, config_path = materialize_config(
        args.config, args.output_dir, config_name=args.config.name,
        resolve_path_keys=("sequence_cache_dir", "flow_schema", "source_dataset_dir", "v1_sidecar_output_dir"),
    )
    ...
    arrays, _ = load_hiqr_v2_arrays(
        ...
        flow_schema=schema, source_dataset_dir=config["paths"]["source_dataset_dir"],
    )
    loader = make_hiqr_v2_loader(
        arrays, "val", batch_size=int(config["evaluation"].get("batch_size", 48)),
        maximum=args.max_sequences, shuffle=False, seed=int(config["evaluation"].get("seed", 42)), num_workers=0,
    )
    first = to_hiqr_v2_batch(next(iter(loader)), loader.field_names, device)
    report = {
        "checkpoint": str(args.checkpoint), "sequences": int(len(loader.dataset)),
        "b0": _b0_probe(model, loader, device),
        "latent": _latent_probe(model, first),
        "continuation": _continuation_probe(model, first),
    }
