#!/usr/bin/env python3
"""Evaluate causal-prior highD semi-Markov rollouts."""
...
def main() -> None:
    ...
    parser.add_argument("--max-sequences", type=int, default=0); parser.add_argument("--log-level", default="INFO")
    args = parser.parse_args(); setup_logging(args.log_level); path = Path(args.config).resolve()
    evaluate_semi_markov_world_model(load_yaml(path), config_dir=path.parent,
        checkpoint=Path(args.checkpoint).resolve() if args.checkpoint else None, max_sequences=args.max_sequences)
