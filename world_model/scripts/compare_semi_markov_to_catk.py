...
def _metrics(pred: np.ndarray, target: np.ndarray, valid: np.ndarray, ego: np.ndarray) -> dict[str, np.ndarray]:
...
def _bootstrap(candidate: np.ndarray, baseline: np.ndarray, *, repetitions: int, seed: int) -> dict[str, Any]:
...
def _relationship_distribution(counts: np.ndarray) -> dict[str, float]:
    ...
    values = np.asarray(counts, np.float64) / total if total else np.zeros(3, dtype=np.float64)
...
def _legacy_sequences(
    source_arrays: dict[str, np.ndarray], sequence_ids: np.ndarray, *, horizon_steps: int, chunks: int,
) -> np.ndarray:
    ...
    for index in range(len(source_arrays["segment_id"])):
        ...
        mode, offset = int(source_arrays["mode_index"][index]), int(source_arrays["offset"][index])
    ...
    for sequence_id in sequence_ids:
        ...
        for chunk in range(1, int(chunks)):
            sequence.append(roll_lookup.get((segment_id, chunk * int(horizon_steps)), -1))
        if min(sequence) < 0:
...
def _catk_multichunk_rollout(
    ...
    *,
    ...
    if horizon * int(chunks) > 125:
        raise ValueError("comparison horizon exceeds the fixed five-second sequence target")
    ...
    with torch.no_grad():
        for chunk in range(int(chunks)):
            action_norm = _model_actions_normalized(
                model, batch, device=device, deterministic=True, temperature=1.0, seed=int(seed) + chunk,
            )
            ...
            generated_local, generated_valid = integrate_background_actions_batch(
                current_raw, current_valid, action_raw, dt=1.0 / float(source_schema["fps"]),
            )
            ...
            target_local = np.asarray(source_arrays["target_states"][current_indices], np.float32)
            ...
            ego_history = np.asarray(source_arrays["ego_future_states"][current_indices], np.float32)
            ego_valid = np.asarray(source_arrays["ego_future_valid"][current_indices], bool)
            local_origin = ego_history[:, -1, :2].copy()
            origins += local_origin
            history = np.zeros((batch_size, horizon, 1 + len(SLOT_NAMES), generated_local.shape[-1]), np.float32)
            ...
            relation = np.stack(
                [
                    build_relation_features_from_current(
                        current_raw[item], current_valid[item],
                        primary_slot_index=int(source_arrays["primary_slot_index"][sequences[item, chunk + 1]]),
                    )
            ...
            batch = {
                "history_states": torch.from_numpy(normalize_states(history, history_valid, source_schema)).float().to(device),
                "history_valid": torch.from_numpy(history_valid).bool().to(device),
                "current_states": torch.from_numpy(normalize_states(current_raw, current_valid, source_schema)).float().to(device),
                ...
                "primary_slot_index": torch.from_numpy(source_arrays["primary_slot_index"][next_indices]).long().to(device),
                "flow_action_summary": torch.zeros(
                    (batch_size, len(SLOT_NAMES), len(source_schema["flow_action_summary_features"])),
...
def main() -> None:
    ...
    if chunks * horizon_steps != int(args.horizon_seconds) * int(source_schema["fps"]):
        raise RuntimeError("comparison horizon must be an exact multiple of the frozen CAT-K chunk length")
    ...
    source_sequences = _legacy_sequences(
        source_arrays, np.asarray(semi_arrays["sequence_id"])[sequence_index],
        horizon_steps=horizon_steps, chunks=chunks,
    )
    ...
    with torch.no_grad():
        for start in range(0, len(sequence_index), int(args.batch_size)):
            ...
            valid = np.asarray(semi_arrays["agent_valid"][seq_idx, 25 : 25 + frames, 1:], bool)
    ...
    output_dir = ensure_dir(Path(semi_cfg["paths"]["output_dir"]).resolve() if Path(semi_cfg["paths"]["output_dir"]).is_absolute() else (semi_path.parent / semi_cfg["paths"]["output_dir"]).resolve())
    ...
    if chunks > 1:
        ...
        relationship_report = {
            "candidate": {"predicted": candidate_relation, "target": target_relation, "total_variation": candidate_tv},
            "baseline": {"predicted": baseline_relation, "target": target_relation, "total_variation": baseline_tv},
            "candidate_minus_baseline_total_variation": float(candidate_tv - baseline_tv),
        }
    report = {
        "protocol": {
            "same_sequence": True,
            ...
            "promotion_information_symmetric": False,
        },
    ...
    path = output_dir / ("paired_semi_markov_vs_catk.json" if chunks == 1 else f"paired_semi_markov_vs_catk_{int(args.horizon_seconds)}s.json")
