"""Formal 8×4 EVT Flow × FIRM-WM composition test."""
...
from .train import load_firm_checkpoint
...
def _batch(starts: dict[str, np.ndarray], donors: np.ndarray, cache: dict[str, np.ndarray], device) -> tuple[dict[str, Any], np.ndarray, np.ndarray]:
    ...
    return {
        ...
        "flow_latent": tensor(starts["features"], device),
    }, ego, present[:, 1:]


def evaluate_flow_composition(*, checkpoint: Path, output_dir: Path) -> dict[str, Any]:
    """Write the single canonical stochastic Flow × FIRM result JSON."""
    import torch
    ...
    for start in range(0, len(donors), batch_size):
        ...
        with torch.no_grad():
            rollout = model.rollout_roll_mode(repeat_batch(batch, INNER_WORLD_SAMPLES), seed=FLOW_COMPOSITION_SEED + 10_000 + start, deterministic=False)
        ...
        reference.append(cache["agent_states"][donors[start:stop], 25:, 1:])
        reference_ego.append(cache["agent_states"][donors[start:stop], 25:, 0])
        reference_valid.append(cache["agent_valid"][donors[start:stop], 25:, 1:])
    ...
    report = {
        "protocol": {
            "name": "held-out EVT Flow × FIRM composition", "outer_flow_samples": 8,
