...
from world_model.src.relations.relational_encoder import RelationalEncoderConfig, RelationalTrafficEncoder
from .state import SemiMarkovConfig, SemiMarkovLatentState
...
class SemiMarkovWorldModelConfig:
    ...
    use_start_flow_summary: bool = True
...
class SemiMarkovRelationalWorldModel(nn.Module):
    ...
    def __init__(self, cfg: SemiMarkovWorldModelConfig) -> None:
        ...
        if self.uses_behavior_anchor:
            self.start_mode = BehaviorAnchorControlPlan(
                physics_steps=cfg.behavior_anchor_response_steps * cfg.physics_steps_per_response,
            )
    ...
    def _batch_behavior_anchor(self, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor | None, torch.Tensor | None, torch.Tensor | None]:
        if not self.uses_behavior_anchor or not self.cfg.use_start_flow_summary:
            return None, None, None
        cached = {
            "raw": batch.get("flow_action_summary"),
            "std": batch.get("flow_action_summary_normalized"),
            "valid": batch.get("flow_action_summary_valid"),
        }
        if all(value is not None for value in cached.values()):
            anchor_raw, anchor_std, anchor_valid = cached["raw"], cached["std"], cached["valid"].bool()
            ...
            if self.frozen_flow_schema is not None:
                raise RuntimeError("the frozen Flow schema requires explicit flow_action_summary tensors")
