...
def _sairl_kinematic_row(
    ...
    *,
    ...
    return [
        ...
        _normalise(y, -lane_width * max(1, lanes_count), lane_width * max(1, lanes_count)),
        _normalise(vx, -2.0 * max_speed, 2.0 * max_speed),
        _normalise(vy, -2.0 * max_speed, 2.0 * max_speed),
    ]


class SAIRLEgoVehicle(MDPVehicle):
    """highway-env MDP vehicle controlled by a loaded SAIRL policy."""
    ...
    def _observation(self) -> np.ndarray:
        rows = [_sairl_kinematic_row(self, origin=None, lanes_count=self.lanes_count)]
        if self.target_vehicle is not None:
            rows.append(
                _sairl_kinematic_row(
                    self.target_vehicle,
                    origin=self,
        ...
        return np.asarray(rows[:7], dtype=np.float32).reshape(-1)
            ...
            trace.append(
                {
                    ...
                    "ego_sairl_action_index": float(
                        getattr(ego, "last_sairl_action_index", 1)
                    ),
