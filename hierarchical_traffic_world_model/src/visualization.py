...
def _summary_figures(output: Path) -> dict[str, str]:
    ...
    if evaluation.get("evaluation_schema_version") != 2:
        raise RuntimeError(
            ...
            "run evaluate_world_model.py before visualizing results"
        )
...
def _reconstruction_outputs(
    ...
    model, _ = load_checkpoint(
        output / "checkpoints/best_hierarchical_world_model.pt", device=device
    )
    generated = rollout(
        model,
        states,
    ...
    save_json(
        {
            ...
            "checkpoint": "checkpoints/best_hierarchical_world_model.pt",
            "frame_stride": int(frame_stride),
            "fps": int(fps),
