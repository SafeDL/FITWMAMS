...
CHECKPOINT_SCHEMA = "hierarchical_traffic_world_model"
...
def train_world_model(config: dict[str, Any], *, config_dir: Path) -> dict[str, Any]:
    ...
    if initial_checkpoint:
        loaded, _ = load_checkpoint(initial_checkpoint, device=device)
        ...
        # cohort.
        factual_reference, _ = load_checkpoint(initial_checkpoint, device=device)
    ...
    factual_relative_tolerance = training.get("factual_relative_tolerance")
    if factual_relative_tolerance is not None and factual_reference is None:
        raise ValueError("factual_relative_tolerance requires initial_checkpoint")
    ...
    best_path = checkpoints / "best_hierarchical_world_model.pt"
    candidate_path = checkpoints / "best_candidate.pt"
    ...
    for epoch in range(1, epochs + 1):
        ...
        factual_score = closed_metrics["ADE_m"] + 0.25 * closed_metrics["FDE_m"]
        selection = factual_score
        if qualified:
            selection = (
                validation_terms["intervention"]
                + float(training["selection_factual_weight"]) * factual_score
                + float(training["selection_jerk_weight"]) * validation_terms["jerk"]
                + float(training.get("selection_energy_weight", 1.0))
                * validation_terms["energy"]
            )
        ...
        history.append(row)
        save_json({"epochs": history}, output / "training_history.json")
    ...
    summary = {
        ...
        "selection_metric": (
            "weighted intervention, factual and jerk score among checkpoints "
            "inside the declared factual-fidelity budget"
        ),
        ...
        "selection_jerk_weight": float(training["selection_jerk_weight"]),
        "selection_energy_weight": float(training.get("selection_energy_weight", 1.0)),
        "best_checkpoint": "checkpoints/best_hierarchical_world_model.pt",
    }
    save_json(summary, output / "training_summary.json")
    shutil.rmtree(preview_root)
    return summary
