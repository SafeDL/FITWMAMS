...
| `highd_natural_driving_evt_same_rear_excluded/` | 相同自然片段排除 `same_rear` 后重新计算的 SEI-EVT | 当前世界模型测试与 IDM/AMS 风险标尺 |
...
全槽位 `highd_natural_driving_evt/` 保留为训练数据来源，不再作为当前 IDM 评估阈值；
它与 `highd_natural_driving_evt_same_rear_excluded/` 的结果不得混合。
