...
`normalizing_flow/` 对 `process_highD/` 已筛出的 highD 长尾自然驾驶事件做条件密度建模，并从该分布中采样新的长尾事件初始状态。它不是 EVT 风险模型，也不重新判断一个片段是否属于长尾；输入数据已经全部来自上游 EVT-tail 事件集合。
...
归一化流采样函数 `normalizing_flow.src.sampling.sample_tail_c0()` 返回一批完整初始条件。对第 `i` 个样本，世界模型正式环境只读取：

```python
feature_row = samples["features"][i]                 # [76]
slot_mask = samples["slot_mask"][i]                   # [6]
primary_slot_index = int(samples["primary_slot_index"][i])
```

其中 76 维连续特征已经包含 ego 当前动力学、六个 slot 的当前相对状态和每个活跃 slot 的未来 1 秒动作摘要。`slot_mask` 与 `primary_slot_index` 是离散事件结构，不能从连续特征中省略或重新猜测。
...
`to_ads_initialization()` 与 `to_world_model_start_condition()` 只生成便于审查或外部系统阅读的字典，不是 `CATKBackgroundEnvironment` 的输入类型。正式环境以 `features`、`slot_mask` 和 `primary_slot_index` 三者为准。

归一化流数据构造与其他模块共用 `process_highD.src.preprocess.prepare_recording()`，因此从原始 highD 读取到 Flow 训练和世界模型训练保持相同的行驶方向、异常轨迹和帧率处理。

## 数学建模

上游 `process_highD/` 已经完成 EVT-tail 筛选，因此 `normalizing_flow/` 不再建模“一个自然驾驶片段是否属于长尾”。这里的统计对象是已经筛好的长尾数据集：
...
因此，`normalizing_flow/` 的角色不是替代世界模型，也不是替代 EVT 风险模型，而是为 ADS 长尾测试和世界模型 START 条件提供可采样、可审计的长尾初始交通条件。

## 数据集与特征设计

### 时间与坐标

每个样本来自一个 highD 自然驾驶长尾片段的 anchor 时刻。上游 `process_highD/` 当前配置为 $25$ Hz、$6$ 秒窗口；`normalizing_flow/` 在 anchor 时刻抽取初始局部状态，并额外抽取 anchor 后 $1$ 秒内的邻车动作摘要。
...
对应实现是：`process_highD/src/preprocess.py` 的 `normalize_driving_direction` 只翻转纵向 $x$ 坐标、纵向速度和纵向加速度；`process_highD/src/natural_segments.py` 的 `_lateral_sign` 和 `_adjacent_lanes_ego_left` 负责把车道关系解释成 ego 左/右；`normalizing_flow/src/metrics.py` 会在生成后检查 `left_*` 与 `right_*` 的横向符号是否满足上述拓扑约束。
...
图片文件保存在 `normalizing_flow/assets/normalizing_flow_structure.png`，用于 README 和论文草稿中的结构说明。
...
- `--config`：默认 `normalizing_flow/scripts/configs/highd_tail_flow_best.yaml`。
